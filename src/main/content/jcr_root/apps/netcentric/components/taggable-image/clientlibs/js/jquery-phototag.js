	...
	$.fn.photoTag = function( options ){
		var defaultOptions = {
			...
			imageWrapBox: {
				...
				tagListRemoveItemIdPrefix: 'photoTag-removeTag_',
		...
		var registerEventsForDeleteLink = function( link, image ){
			link.click(
				function(e){
					e.preventDefault();
					var uri = link.attr('href').substring(1);
                    var tagId = this.id.substring(this.id.lastIndexOf('_') + 1);
		...
		var createNewTagForm = function( tempTagBox, image, image_id ){
			...
			$('#tempNewTagForm').submit(function(e){
                ...
                var id = Date.now(); // FIXME: get a more unique ID
				$.post(options.addTagUrl+'?'+$.param(tag) + '&' + $(this).serialize() + '&jcr:primaryType=nt:unstructured&id=' + id,
                    null,function(response){
					if(response.result != undefined && !response.result){
						manageError(response);
						return;
					}
					$.getJSON(response.location + '.json', function(response){
						var tagBox = createTagBoxFromJSON(response,image);
						$('#' + options.imageWrapBox.idPrefix + image_id).append(tagBox);
						extendTagBoxAttributes(tagBox,response,image,image_id);
                    });
				},'json');
				removeNewTempTag();
				showAllTags(image_id);
		...
		var createTagBoxFromJSON = function( tagJSON, image ){
			...
			if(options.isEnabledToEditTags && tagJSON.isDeleteEnable && options.tag.showDeleteLinkOnTag){
				var deleteLink = $('<a id="'+ options.tag.deleteLinkIdPrefix + tagJSON.id +'" class="'+ options.tag.deleteLinkCssClass +'" href="#'+ tagJSON.uri +'"></a>');
		...
		var createTagItemForList = function( tagJSON, image ){
			var item = $('<li></li>');
			if(tagJSON.text_id){
				var link = $('<a href="'+ tagJSON.text_id + '/profile.form.html/content/geometrixx-outdoors/en/user/social-profile">'+ tagJSON.text +'</a>');
				item.append(link);
			}else{
				item.append(tagJSON.text);
			}
			if(tagJSON.isDeleteEnable){
				var deleteLink = $('<a id="'+ options.imageWrapBox.tagListRemoveItemIdPrefix + tagJSON.id +'" class="'+ options.tag.deleteLinkCssClass +'" href="#'+ tagJSON.uri +'">'+ options.literals.removeTag +'</a>');
				registerEventsForDeleteLink(deleteLink,image);
				item.append(' (');
				...
				$.getJSON(
					...
					function( response ){
                        ...
                        var obj = {
                            "Image" : [
                                {
                                    "id":150, // TODO: get id from somewhere
                                "Tags": response
                                }
                            ],
                            "options":{
						...
						if(obj.options){
							options = $.extend(true,options,obj.options);
						}
